...
>  ## 1.4. 기타 키워드 정리 
>  - ***Booleans888 : True/False만 판단하는 논리값
...
>  plus(5, 5);
>  ```
>  - 그런데 이때, console.log는 함수의 결과값을 콘솔창에 표시하는 역할을 함. 하지만 함수의 결과값을 콘솔창에 표시하는 게 아니라 코드 안에서 가져오게 할려면? ***결과값을 반환해주는 return 키워드*** 를 사용해야 함. (참고로 return이 되는 순간 함수가 종료되기 때문에 return 뒤에 오는 코드들은 실행되지 않음.)
>  
>  ```js
